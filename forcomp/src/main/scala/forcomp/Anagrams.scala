...
object Anagrams {
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    x.map { case(char, count) => (char, count - y.toMap.getOrElse(char, 0)) }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    ...
    def anagrams(occurrences: Occurrences): List[Sentence] = {
      occurrences match {
        case Nil => List(List()) // 빈 리스트에 대해
        case _ =>
          for {
            comb  <- combinations(occurrences)                     // 가능한 조합을 찾아서
            words <- dictionaryByOccurrences.getOrElse(comb, Nil)  // 그 조합에 해당하는 단어를 찾고
            tail  <- anagrams(subtract(occurrences, comb))         // 남은 문자들에 대해 recursive
          } yield words :: tail
      }
    }
  ...
  def sentenceAnagramsMemo(sentence: Sentence): List[Sentence] = {
    ...
    def anagrams(occurrences: Occurrences): List[Sentence] = {
      if (memo.contains(occurrences)) memo(occurrences) // 이미 계산된 결과가 있으면 반환
      else occurrences match {
        case Nil => List(List())
        case _ =>
          val result = for {
            comb  <- combinations(occurrences)
            words <- dictionaryByOccurrences.getOrElse(comb, Nil)
            tail  <- anagrams(subtract(occurrences, comb))
          } yield words :: tail
          memo += (occurrences -> result) // 계산된 결과를 캐시에 저장
          result
      }
    }
