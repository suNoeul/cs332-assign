...
object NodeScala {
  ...
  trait Exchange {
    ...
    /** Communicates that the response has ended and that there will be no further writes. */
    def close(): Unit
  ...
  trait Listener {
     ...
     *  2) installs an asynchronous `HttpHandler` to the `server` that deregisters itself
